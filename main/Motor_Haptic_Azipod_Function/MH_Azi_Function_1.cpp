...
void MotorHaptic::loopAF1() {
    ...
    motor.loopFOC();
    // printf("Angle: %f deg, Velocity: %f deg/s\n", motor.shaft_angle*RAD_TO_DEG, motor.shaft_velocity*RAD_TO_DEG);
    ...
    if (fabs(error-main_point_haptic) < angle_dead_zone) {
        ...
        if(one_time) {
            ...
            motor.P_angle.P = haptic_default_pid_p;
            motor.P_angle.D = haptic_default_pid_d;
            motor.PID_velocity.P = FOC_AZI_PID_PV_DEFAULT;
            // motor.P_angle.D = haptic_default_pid_d;
            motor.P_angle.reset();
            motor.PID_velocity.reset();
            // motor.controller = MotionControlType::angle;
        ...
        // if(fabs(error-main_point_haptic) < 0.1f*DEG_TO_RAD) {
        //     flag_inc = true;
        // }


        // if(fabs(error-main_point_haptic) < 0.3f*DEG_TO_RAD && fabs(motor.shaft_velocity) < 0.1f) {
        //     cnt++;
        //     if(cnt > 500 && flag_inc) {
        ...
        //     cnt =0;
        // }
        // if(fabs(error-main_point_haptic) < 0.075f*DEG_TO_RAD) {
        //     motor.PID_velocity.P = 0.2;
        //     motor.P_angle.reset();
        //     motor.PID_velocity.reset();
        //     // motor.P_angle.P = 0;
        // } else if(fabs(error-main_point_haptic) > 0.45f*DEG_TO_RAD) {
        //     motor.PID_velocity.P = FOC_AZI_PID_PV_DEFAULT;
        //     // motor.P_angle.P = haptic_default_pid_p;
        // }
///////////////////////////////////////////////////////////////////////////////////////////////////////////////        
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////  
        ...
        float current_sp = motor.P_angle(home_angle - sub_point_haptic - current_angle);
...
//     // motor.move(5);
//     float error = home_angle - motor.shaft_angle;
//     float error1 = home_angle_1 - motor.shaft_angle;
//     float error2 = home_angle_2 - motor.shaft_angle;
//     float main_point_haptic = round(error/degree_90)*degree_90;
//     float sub_point_haptic = round(error/degree_5)*degree_5;
//     //printf("Error: %f deg, Main point: %f deg\n", error*RAD_TO_DEG, main_point_haptic*RAD_TO_DEG);
//     if (error1 < angle_dead_zone && error1 > 0) {
//         if(one_time) {
//             motor.P_angle.P = haptic_default_pid_p;
//             // motor.P_angle.D = haptic_default_pid_d;
//             motor.P_angle.reset();
...
//         float shaft_velocity_sp = motor.P_angle(home_angle_1 - current_angle);
//         shaft_velocity_sp = _constrain(shaft_velocity_sp, -motor.velocity_limit, motor.velocity_limit);
//         float current_sp = motor.PID_velocity(shaft_velocity_sp - current_velocity);
//         // current_sp = current_sp - 1.2 * current_velocity;
//         current_sp = _constrain(current_sp, -11.4f, 11.4f);
